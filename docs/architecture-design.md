...
> ContextAgent 采用**分层单体 + 异步事件**架构风格，以 openJiuwen Python SDK 为基础框架，通过适配器层隔离对外部存储（向量库、图数据库、KV 缓存）和 openJiuwen 原生组件的直接依赖。系统核心技术栈为 Python 3.11+ / asyncio / FastAPI / openJiuwen / Redis / Chroma（或 Milvus）。首要质量目标是**关键路径上下文召回 P95 ≤ 300ms**，同时保证可插拔的压缩/检索策略以适应不同业务场景。
...
- **系统外（外部依赖）**：openJiuwen 框架（LongTermMemory、ContextEngine、Retriever 等）、向量数据库（Chroma/Milvus）、图数据库（可选）、Redis（KV 缓存）、对象存储（S3 兼容）、LLM 服务（压缩/摘要调用）、消息队列（Pulsar，分布式部署可选）、监控存储（Prometheus/InfluxDB）
...
| 向量数据库（Chroma/Milvus） | 双向 | HTTP/gRPC | JSON/Protobuf | 向量存储与检索 |
...
| **基础设施层** | 存储、计算、消息队列 | openJiuwen（LTM/CE/Retriever/TMS）、Redis、Chroma/Milvus、图数据库、对象存储、Pulsar |
...
| 向量数据库 | Chroma（默认）/ Milvus（生产） | Chroma 0.5+ / Milvus 2.4+ | 向量存储与语义检索 | openJiuwen 已支持，Chroma 轻量嵌入，Milvus 生产级 |
...
| 包管理 | uv | 0.4+ | 依赖管理/虚拟环境 | openJiuwen 已使用（见 uv.lock） |
...
graph TB
    ...
    subgraph StorageLayer["存储层"]
        ...
        VEC[Chroma 0.5 / Milvus 2.4<br/>向量存储]
...
| 集成测试 | UC 端到端（真实 Redis/Chroma） | 关键路径 ≥ 90% | pytest + testcontainers |
...
graph LR
    subgraph CI["CI 流水线（每次 Push/PR）"]
        ...
        UnitTest --> IntTest[集成测试<br/>testcontainers Redis+Chroma]
...
```makefile
# Makefile
lint:       uv run ruff check . && uv run mypy context_agent/
test:       uv run pytest tests/unit/ -v --cov=context_agent --cov-report=term
test-int:   uv run pytest tests/integration/ -v
test-perf:  uv run pytest tests/performance/ -v --benchmark-only
...
| Chroma（向量库，轻量） | 2 核 | 8 GB | 100 GB SSD | 本地嵌入部署 |
...
| 向量库 | Chroma（嵌入） | Chroma（独立） | Milvus 集群 |
...
| 向量库 | Chroma（嵌入进程） | Chroma（独立进程） | Milvus 集群（3节点） |
